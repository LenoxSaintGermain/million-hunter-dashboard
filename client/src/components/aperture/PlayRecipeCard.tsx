...
export function PlayRecipeCard({
  ...
  return <section className="overflow-hidden rounded-xl border" style={{ borderColor: "var(--sh-signal)", background: "var(--sh-surface-2)" }}>
    ...
    {noPlay ? <div className="space-y-4 p-4 sm:p-5">
      ...
      <div className="flex flex-wrap gap-2"><Button type="button" size="sm" className="min-h-11" onClick={exactOptionTicketCanResolve ? onPrepareProposal : onReviewEvidence}>{exactOptionTicketCanResolve ? "Open practice order" : "Resolve blocker"}</Button><Button type="button" variant="ghost" size="sm" className="min-h-11" onClick={onOpenResearch}>View research</Button></div>
