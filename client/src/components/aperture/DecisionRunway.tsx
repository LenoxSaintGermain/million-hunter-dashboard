...
export function DecisionRunway({ onNewResearch, onOpenResearchRun, receiptTarget = null, missionHandoff = null, onMissionRecorded }: Props) {
  ...
  const handoffReadsLoading = !!handoff && (canonicalQuery.isLoading || capitalQuery.isLoading || draftQuery.isLoading
  ...
  const [selectedAccountId, setSelectedAccountId] = useState<number | null | undefined>(handoff ? null : undefined);
  ...
  const activeCanonicalId = handoff ? handoff.canonicalThesisId : preservedStrategyDraft ? preservedStrategyDraft.values.canonicalThesisId
  ...
  const projection = useMemo(() => handoff ? handoffProjection ?? null : immutableReceipt
    ? (capitalTheses ?? []).find((item) => item.id === immutableReceipt.capitalThesisId) ?? null
    : (capitalTheses ?? []).find((item) => item.sourceCompilationId === activeCanonicalId) ?? null, [capitalTheses, activeCanonicalId, immutableReceipt, handoff, handoffProjection]);
  ...
  const [editingCompletedMission, setEditingCompletedMission] = useState(false);
  ...
  const setExpandedMissionSection = (section: 1 | 2 | 3) => {
    markDraftEdited();
    dispatchMissionSection({ type: "choose", section });
    setSectionFocusRequest({ section });
  };
  useEffect(() => {
    if (!sectionFocusRequest) return;
    // Consume only an explicit navigation request, after the section is visible.
    // Hydration and provider updates never enqueue or replay this handoff.
    setSectionFocusRequest(null);
    const headingId = sectionFocusRequest.section === 1 ? "mission-section-thesis"
      : sectionFocusRequest.section === 2 ? "mission-section-risk" : "mission-section-review";
  ...
  const missionContextError = handoffError || (receiptStructureInvalid ? new Error("Saved Mission horizons are malformed. Reload the corrected receipt; no new analysis can start.") : receiptError || draftQuery.error || canonicalQuery.error || capitalQuery.error || accountQuery.error);
  const missionContextLoading = !receiptTarget && (handoffReadsLoading || !draftInitialized || receiptLoading || draftQuery.isLoading || canonicalQuery.isLoading || capitalQuery.isLoading || accountQuery.isLoading);
  ...
  const rawUnderwritingResult = runUnderwriting.data ?? (currentDecisionRunId != null ? currentUnderwriting.data : null) ?? null;
  ...
  const chooseMission = (item: NonNullable<typeof library.data>[number]) => {
    ...
    // A waiting/no-allocation disposition pauses the target, not its saved value.
    ...
    try {
      ...
      if (branch === "research") {
        ...
        try {
          ...
          setEditingCompletedMission(false);
        } catch (error: any) {
        ...
        setRevisingReceipt(false);
        toast.success(branch === "cash" ? "No new allocation recorded. Existing positions remain unchanged." : "Conditional review queued");
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    const persisted = receiptTarget ? null : draftQuery.data ?? null;
    savedDraftRef.current = persisted;
    setSavedDraft(persisted);
    setDraftInitialized(true);
    if (handoff && handoffSource && handoffProjection) {
      if (persisted?.completedAt == null && persisted?.values.canonicalThesisId === handoff.canonicalThesisId && !persisted.values.strategyContext) {
        applyDraftValues(persisted.values);
      } else {
        try { applyDraftValues(canonicalMissionHandoffValues(handoffSource, handoffProjection)); }
        catch { setDraftError("Saved thesis declarations are malformed. Review the source before starting this Mission."); draftFailureRef.current = "Saved thesis declarations are malformed."; return; }
        if (persisted && persisted.completedAt == null) {
          const message = "An unfinished Mission is already saved. Compare it with the requested thesis before choosing which draft to continue.";
    ...
    setCapital(String(receipt.deployableCapitalCents / 100));
    ...
    setMaxLoss(String(receipt.maxPlannedLossCents / 100));
  ...
  const flushDraft = async (): Promise<MissionDraftRecord | null> => {
    ...
    if (!draftInitialized || missionContextError || handoffReadsLoading || draftFailureRef.current) throw new Error(draftFailureRef.current ?? "Wait for the saved Mission draft to load.");
    ...
    const request = saveDraftMutation.mutateAsync({ expectedVersion: previous?.version ?? 0, values,
      ...(handoff && previous?.values.strategyContext ? { replaceStrategyContext: true } : {}) });
  ...
  const useRemoteDraft = () => {
    ...
    if (handoff) { window.location.assign(aperturePathForFixture("/aperture/mission", readIsolatedUatIdentity())); return; }
  ...
  if (!receiptTarget && !draftInitialized && missionContextError) {
    if (handoffError) return <section role="alert" className="mx-auto max-w-3xl rounded-2xl border p-5" style={{ borderColor: "var(--sh-red)", background: "var(--sh-surface)" }}><h2 className="font-serif text-2xl">Thesis handoff unavailable</h2><p className="mt-2 text-sm leading-6">{handoffError}</p><Button className="mt-4 min-h-11" onClick={() => void refreshMissionContext()}>Retry loading saved context</Button></section>;
  ...
  // Completion is determined by the persisted revision and matching inputs, not
  // a device-local onboarding flag. Read-only editing disclosure does not revise it.
  if (underwritingComplete && underwritingResult && !editingCompletedMission && !draftError && !missionContextError) {
    return <MissionResultWorkspace result={underwritingResult} accountLabel={paperAccount?.label ?? "Paper account unavailable"} accountAsOf={paperAccount?.lastSyncedAt ?? null} thesisLabel={activeThesis ? canonicalThesisLabel(activeThesis) : "Saved thesis"} revisionLabel={`mission v${latestRevision ?? "—"}`} selectedPlayId={underwritingResult.selectedPlayId} busy={busy}
      notice={paperAccount?.lastSyncedAt == null || Date.now() - paperAccount.lastSyncedAt > STALE_ACCOUNT_MS ? <p role="status" className="rounded-lg border p-3 text-sm" style={{ borderColor: "var(--sh-signal)" }}>Account snapshot is stale or unavailable. Current capacity is unverified.</p> : authoritativePreview.isError || cockpit.isError ? <p role="alert" className="rounded-lg border p-3 text-sm" style={{ borderColor: "var(--sh-signal)" }}>Constraint refresh failed. Inspect constraints; current capacity is unverified.</p> : null}
      riskDetails={<MissionRiskInspection context={riskInspection} enteredLossCents={parseMoney(maxLoss)} policyVersion={cockpit.data?.mandate.version ?? null} onRefresh={refreshRiskInspection} />}
      onEdit={() => setEditingCompletedMission(true)} onValidate={validateUnderwrittenPlay} />;
  }
  ...
  return <section className="mx-auto max-w-5xl space-y-5 pb-24" onChangeCapture={markDraftEdited} onClickCapture={(event) => { if (event.target instanceof Element && event.target.closest("[data-draft-edit]")) markDraftEdited(); }}>
    ...
    {activeThesis && <div className="space-y-4">
      <article className="overflow-hidden rounded-2xl border" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface)" }}>
        ...
        {receiptActive ? <DecisionReceipt branch={latestBranch as "cash" | "conditional"} reason={latestReason} blocker={latestBlocker} reopen={latestReopen} gateLabel={latestGate} reviewAt={latestReviewAt} revision={latestRevision} recordedAt={latestRecordedAt} binding={immutableReceipt?.binding} pendingCashOutcome={currentCashOutcome} recordedCashOutcome={authoritativeLatest?.cashOutcome} onRecordCashOutcome={recordCashOutcome} resolvingCashOutcome={resolveCashOutcome.isPending} onGateReview={reviseReceipt} onRevise={reviseReceipt} /> : <><div className="space-y-5 p-5 sm:p-7">
          ...
          <div hidden={visibleMissionSection !== 2} className="space-y-5"><section id="mission-math" aria-labelledby="mission-section-risk"><div className="mb-2 flex items-center justify-between gap-3"><div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.14em]" style={{ color: visibleMissionSection === 2 ? "var(--sh-signal)" : "var(--sh-fg-muted)" }}>2 · Account & risk</p><h2 id="mission-section-risk" tabIndex={-1} className="mt-1 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4" style={{ scrollMarginTop: "calc(var(--header-height, 4rem) + 1rem)" }}>Confirm the capital and loss boundary.</h2></div><span className="text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ color: "var(--sh-fg-muted)" }}>Operator-declared inputs</span></div><div className="grid overflow-hidden rounded-xl border sm:grid-cols-2 lg:grid-cols-4" style={{ borderColor: "var(--sh-border-1)" }}>
            ...
            {branch === "research" ? <TargetProfitField value={targetProfit} onChange={(value) => { setTargetProfit(value); setUnderwritingDirty(true); }} period={targetPeriod} onPeriodChange={(value) => { setTargetPeriod(value); setUnderwritingDirty(true); }} /> : <div className="border-b p-3 text-[0.68rem] sm:border-b-0 sm:border-r" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}>Profit target<p className="mt-1 font-serif text-xl" style={{ color: "var(--sh-text-primary)" }}>Not used</p><span className="text-[10px]">{branch === "cash" ? "No new allocation; existing positions unchanged." : "Gate review, not return target."}</span></div>}
        ...
        {missionConfigured && authoritativePreview.isError && !underwritingComplete && <p role="alert" className="rounded-lg border p-3 text-xs leading-5" style={{ borderColor: "var(--sh-red)", color: "var(--sh-red)" }}>The effective portfolio constraint could not be verified. Your current inputs remain visible; check draft status above. Underwriting is withheld until the constraint can be verified.</p>}
          ...
          <div hidden={visibleMissionSection !== 3}><section id="mission-disposition" aria-labelledby="mission-section-review" className="rounded-xl border p-4" style={{ borderColor: "var(--sh-signal)", background: "var(--sh-surface-2)" }}><p className="text-[0.62rem] font-semibold uppercase tracking-[0.14em]" style={{ color: "var(--sh-signal)" }}>3 · {reviewSectionLabel}</p><h2 id="mission-section-review" tabIndex={-1} className="mt-1 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4" style={{ scrollMarginTop: "calc(var(--header-height, 4rem) + 1rem)" }}>{branch === "research" ? "Confirm the mission before analysis." : branch === "conditional" ? "Confirm the review condition." : "Confirm no new allocation."}</h2><dl className="mt-3 grid gap-px overflow-hidden rounded-lg border text-xs sm:grid-cols-3" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-border-1)" }}><ReceiptFact label="Thesis" value={canonicalThesisLabel(activeThesis)} /><ReceiptFact label="Capital / instruments" value={`${formatCents(capitalCents)} · ${instrument === "either" ? "Shares or options" : instrument}`} /><ReceiptFact label={branch === "research" ? "Target / effective risk" : "Decision / effective risk"} value={`${branch === "research" ? explicitTargetProfitCents != null ? `${formatCents(explicitTargetProfitCents)} / ${targetPeriod}` : "No explicit target" : branch === "conditional" ? "Conditional review" : "No new allocation"} · ${effectiveRiskSummary}`} /></dl><p className="mt-3 text-xs leading-5" style={{ color: "var(--sh-fg-muted)" }}>{dispositionEffect}</p>
            ...
            <MissionReviewFeasibility branch={branch} feasibility={feasibilityForDisplay} enteredLossCents={parseMoney(maxLoss)} normalPolicyPct={cockpit.data?.mandate.maxPlannedRiskPctPerPlay ?? null} policyVersion={cockpit.data?.mandate.version ?? null} accountCeilingCents={plannedRiskCeiling} openRiskCents={previewPortfolioRisk?.beforeCents ?? null} remainingHeadroomCents={previewPortfolioRisk?.remainingHeadroomCents ?? null} inspection={riskInspection} onInspect={refreshRiskInspection} />
        ...
        {!underwritingComplete && <footer className="flex flex-col gap-3 border-t p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface-2)" }}>
          ...
          <Button id="mission-primary-action" className="min-h-11 w-full sm:w-auto" disabled={busy || primaryActionBlocker != null || persistedUnderwritingLoading || underwritingComplete || persistedUnderwritingUnavailable} onClick={commit}>{runUnderwriting.isPending ? "Underwriting…" : saveMission.isPending ? "Saving mission…" : persistedUnderwritingLoading ? "Checking saved underwriting…" : persistedUnderwritingUnavailable ? "Saved result unavailable" : branch === "cash" ? "Record no new trade" : branch === "conditional" ? "Queue conditional review" : "Underwrite my mission"}<ArrowRight className="ml-2 h-4 w-4" /></Button>
        </footer>}</>}
      </article>

      <details className="rounded-xl border" style={{ borderColor: "var(--sh-border-1)" }}>
      <summary className="min-h-11 cursor-pointer px-4 py-3 text-sm font-semibold">Other mission ideas &amp; context</summary>
      <aside className="space-y-4 border-t p-3" style={{ borderColor: "var(--sh-border-1)" }}>
...
export function MissionRiskInspection({ context, enteredLossCents, policyVersion, onRefresh }: { context: MissionRiskInspectionContext; enteredLossCents: number; policyVersion: string | null; onRefresh: () => void }) {
  ...
  return <details data-risk-inspection className="mt-3 rounded-lg border" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface-2)" }}>
    ...
    <div className="space-y-3 border-t p-3 text-sm leading-6" style={{ borderColor: "var(--sh-border-1)" }}>
      ...
      <p>Your entered loss limit has not been changed. Account selection, approvals, and orders remain unchanged.</p>
