  ...
  return <section className="mx-auto max-w-[1440px] space-y-5 pb-24">
    ...
    {activeThesis && <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_23rem]">
      <article className="overflow-hidden rounded-2xl border" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface)" }}>
        ...
        <nav aria-label="Mission to submission path" className="border-b" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface-2)" }}><ol className="grid grid-cols-4 gap-px text-[0.55rem] font-semibold uppercase tracking-[0.06em] sm:text-[0.62rem] sm:tracking-[0.1em]" style={{ background: "var(--sh-border-1)" }}><li><button type="button" aria-current="step" className="flex min-h-11 w-full items-center justify-center gap-1 px-1 text-center sm:gap-2 sm:px-3" style={{ background: "var(--sh-surface-2)", color: "var(--sh-signal)" }} onClick={() => document.getElementById("mission-math")?.scrollIntoView({ behavior: "smooth", block: "center" })}><span className="font-mono tabular-nums">1</span>Mission</button></li><li><button type="button" disabled={!currentBindingMatches || authoritativeLatest?.runId == null} className="flex min-h-11 w-full items-center justify-center gap-1 px-1 text-center disabled:cursor-not-allowed disabled:opacity-55 sm:gap-2 sm:px-3" style={{ background: "var(--sh-surface-2)", color: currentBindingMatches && authoritativeLatest?.runId != null ? "var(--sh-text-primary)" : "var(--sh-fg-muted)" }} onClick={() => currentBindingMatches && authoritativeLatest?.runId != null && onOpenResearchRun(authoritativeLatest.runId)}><span className="font-mono tabular-nums">2</span>{currentBindingMatches && authoritativeLatest?.runId != null ? "Open Slate" : "Play Slate"}</button></li>{[["3", "Ticket"], ["4", "Submit"]].map(([number, label]) => <li key={label}><button type="button" disabled className="flex min-h-11 w-full cursor-not-allowed items-center justify-center gap-1 px-1 text-center opacity-55 sm:gap-2 sm:px-3" style={{ background: "var(--sh-surface-2)", color: "var(--sh-fg-muted)" }} title={label === "Ticket" ? "Available after you choose a play" : "Available after proposal approval"}><span className="font-mono tabular-nums">{number}</span>{label}</button></li>)}</ol></nav>
