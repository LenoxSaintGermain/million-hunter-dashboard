      ...
      <article className="overflow-hidden rounded-2xl border" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface)" }}>
        ...
        {!underwritingComplete && <footer className="flex flex-col gap-3 border-t p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface-2)" }}>
          <div className="max-w-xl text-sm leading-5" style={{ color: "var(--sh-fg-muted)" }}>
            <p>{persistedUnderwritingLoading ? "Checking saved underwriting. No new analysis starts until its status is known." : persistedUnderwritingUnavailable ? "Saved result unavailable. Refresh its status before new analysis." : branch === "cash" ? "Records no new allocation. Existing positions remain unchanged." : branch === "conditional" ? "Records a review condition. No proposal or broker order." : "Builds a research playbook. Does not create or submit an order."}</p>
