  ...
  return (
    <section className="overflow-hidden rounded-2xl border" style={{ borderColor: blocked ? "color-mix(in oklab, var(--sh-signal) 65%, var(--sh-border-1))" : "oklch(0.55 0.15 145)", background: "var(--sh-surface)" }}>
      ...
      <div className="grid gap-0 divide-y sm:grid-cols-3 sm:divide-x sm:divide-y-0" style={{ borderColor: "var(--sh-border-1)" }}>
        ...
        <div className="p-4"><p className="text-[0.65rem] font-semibold uppercase tracking-[0.13em]" style={{ color: "var(--sh-fg-muted)" }}>Decision-critical checks</p><ol className="mt-2 space-y-2">{focus.humanChecks.map((check, index) => <li key={check} className="flex gap-2 text-sm leading-5" style={{ color: "var(--sh-text-primary)" }}><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[0.65rem] font-semibold" style={{ color: "var(--sh-signal)", background: "color-mix(in oklab, var(--sh-signal) 14%, transparent)" }}>{index + 1}</span>{check}</li>)}</ol><p className="mt-2 text-[11px] leading-4" style={{ color: "var(--sh-fg-muted)" }}>These gate paper-order review only. They do not block research navigation or posture comparison.</p></div>
      </div>
