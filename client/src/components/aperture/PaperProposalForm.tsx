...
const exactMoney = (cents: number | null | undefined) => cents == null
  ? "—"
  : new Intl.NumberFormat(navigator.language, { style: "currency", currency: "USD", minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(cents / 100);
...
export function PaperProposalForm({ runId, candidate, account, run, evidenceReviewComplete = false, onReturnToBrief, onReturnToDecisionBrief, onProposalCreated, onCashPreserved }: {
  ...
  const ticketEntryRef = useRef<HTMLElement>(null);
  const scrolledCandidateRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    if (!candidate?.id || scrolledCandidateRef.current === candidate.id) return;
    scrolledCandidateRef.current = candidate.id;
    const timer = window.setTimeout(() => {
      ticketEntryRef.current?.scrollIntoView({
  ...
  const optionAvailability = marketAvailabilityCopy(
    optionChain.data?.unavailableReason ?? constructedPlay?.unavailableReasons[0],
    {
      ...constructed.data?.marketContext,
      session: optionChain.data?.marketSession ?? constructed.data?.marketContext?.session,
      nextRegularSessionOpenAt: optionChain.data?.nextRegularSessionOpenAt ?? constructed.data?.marketContext?.nextRegularSessionOpenAt,
    },
  );
  ...
  const hardResolutionNeeded = isOption && hardPreflightResult != null && !optionResolutionNeeded && !riskResolutionNeeded;
  const riskCeilingCents = riskCeilingResult?.key === "planned_risk_per_play"
    ...
    && riskCeilingResult.ceiling != null
      ...
      : null;
  const visibleRiskCeilingCents = riskCeilingCents ?? constructedPlay?.budgetCents ?? null;
  const quotedContractLosses = optionChainItems.flatMap((item) => item.market?.askPriceCents != null && Number.isInteger(optionQty) && optionQty > 0 && slippageCents != null
    ? [{ symbol: item.contract.symbol, lossCents: optionQty * 100 * (item.market.askPriceCents + slippageCents) }]
    : []);
  const cheapestQuotedLossCents = quotedContractLosses.length ? Math.min(...quotedContractLosses.map((item) => item.lossCents)) : null;
  const fittingContractCount = visibleRiskCeilingCents == null ? null : quotedContractLosses.filter((item) => item.lossCents <= visibleRiskCeilingCents).length;
  const maximumPremiumPerShareCents = visibleRiskCeilingCents != null && Number.isInteger(optionQty) && optionQty > 0 && slippageCents != null
    ? Math.max(0, Math.floor(visibleRiskCeilingCents / (optionQty * 100)) - slippageCents)
    : null;
  ...
  return <Card id="paper-proposal" className="scroll-mt-6 border" style={{ borderColor: "var(--sh-signal)", background: "var(--sh-surface-2)" }}>
    <CardContent className="space-y-4 pt-6">
      ...
      {isOption && <section ref={(node) => { contractPickerRef.current = node; ticketEntryRef.current = node; }} aria-labelledby="contract-picker-heading" className="scroll-mt-20 space-y-3 rounded-lg border p-3" style={{ borderColor: "var(--sh-signal)", background: "var(--sh-surface)" }}>
        ...
        {visibleRiskCeilingCents != null && <div className="rounded-md border px-3 py-2 text-xs leading-5" style={{ borderColor: fittingContractCount === 0 ? "var(--sh-red)" : "var(--sh-border-1)", background: "var(--sh-surface-2)", color: "var(--sh-fg-muted)" }}><strong style={{ color: "var(--sh-text-primary)" }}>Available headroom: {exactMoney(visibleRiskCeilingCents)}.</strong>{maximumPremiumPerShareCents != null ? <> For {optionQty} contract{optionQty === 1 ? "" : "s"}, premium must be at or below {price(maximumPremiumPerShareCents)} per share before slippage.</> : null}{fittingContractCount != null ? <> {fittingContractCount} of {quotedContractLosses.length} quoted contracts fit.</> : null}{fittingContractCount === 0 && cheapestQuotedLossCents != null ? <> Lowest quoted maximum loss is {exactMoney(cheapestQuotedLossCents)}, over by {exactMoney(cheapestQuotedLossCents - visibleRiskCeilingCents)}. Do not move farther out of the money only to force a fit.</> : null}</div>}
        {optionChain.isFetching ? <div className="flex min-h-20 items-center justify-center gap-2 rounded-md border text-xs" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}><Loader2 className="h-4 w-4 animate-spin" />Loading live contracts…</div> : optionChainItems.length ? <div className="overflow-hidden rounded-md border" style={{ borderColor: "var(--sh-border-1)" }}>
          <div className="hidden grid-cols-[1.05fr_1.3fr_.9fr_.6fr_.55fr_.55fr] gap-2 border-b px-3 py-2 text-[9px] font-semibold uppercase tracking-[0.1em] sm:grid" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}><span>Contract</span><span>Bid / ask · mark</span><span>Maximum loss</span><span>Spread</span><span>Volume</span><span>Open int.</span></div>
          {optionChainItems.map((item) => {
            const selected = optionContractSymbol === item.contract.symbol;
            const rowMaxLossCents = item.market?.askPriceCents != null && Number.isInteger(optionQty) && optionQty > 0 && slippageCents != null ? optionQty * 100 * (item.market.askPriceCents + slippageCents) : null;
            const rowFitsLimit = rowMaxLossCents != null && visibleRiskCeilingCents != null ? rowMaxLossCents <= visibleRiskCeilingCents : null;
            const rowOverageCents = rowFitsLimit === false && rowMaxLossCents != null && visibleRiskCeilingCents != null ? rowMaxLossCents - visibleRiskCeilingCents : null;
            const marketClosed = optionChain.data?.marketSession != null && optionChain.data.marketSession !== "regular";
            const unavailableLabel = marketClosed ? "Market closed" : "Live quote unavailable";
            const unavailableDetail = marketClosed
              ? "Reference only. Live bid/ask and liquidity must sync during the next regular session before this contract can be selected."
              : "Live bid/ask, volume, open interest, volatility, or spread did not pass the quote check.";
            const statusId = `quote-status-${item.contract.symbol}`;
            return <button key={item.contract.symbol} type="button" aria-disabled={!item.quoteReady} aria-describedby={!item.quoteReady ? statusId : undefined} aria-pressed={selected} title={!item.quoteReady ? unavailableDetail : "Select this contract and use its ask as the editable limit"} aria-label={`${candidate.symbol} ${price(item.contract.strikePriceCents)} ${item.contract.type}; ${item.quoteReady ? `bid ${price(item.market?.bidPriceCents)}, ask ${price(item.market?.askPriceCents)}, maximum loss ${money(rowMaxLossCents)}, ${rowFitsLimit === true ? "fits limit" : rowFitsLimit === false ? `over limit by ${exactMoney(rowOverageCents)}` : "risk limit unavailable"}` : unavailableLabel}`} onClick={() => item.quoteReady ? selectQuotedContract(item) : toast.info(unavailableLabel, { description: unavailableDetail })} className={`grid min-h-12 w-full gap-1 border-b px-3 py-2 text-left text-xs last:border-b-0 sm:grid-cols-[1.05fr_1.3fr_.9fr_.6fr_.55fr_.55fr] sm:items-center ${item.quoteReady ? "cursor-pointer" : "cursor-help opacity-55"}`} style={{ borderColor: "var(--sh-border-1)", background: selected ? "color-mix(in srgb, var(--sh-signal) 10%, var(--sh-surface))" : "transparent", color: "var(--sh-text-primary)" }}>
              <span className="font-semibold tabular-nums">{price(item.contract.strikePriceCents)} {item.contract.type}{!item.quoteReady && <small id={statusId} className="mt-1 block font-sans text-[10px] uppercase tracking-[0.08em]" style={{ color: "var(--sh-signal)" }}>{unavailableLabel}</small>}</span>
              <span className="font-mono tabular-nums">{item.market ? <>{price(item.market.bidPriceCents)} / {price(item.market.askPriceCents)}<small className="ml-1 opacity-70">· mark {price(item.midpointCents)}</small></> : "Quote unavailable"}</span>
              <span className="font-mono tabular-nums">{exactMoney(rowMaxLossCents)}<small className="ml-1 font-sans font-semibold" style={{ color: rowFitsLimit === true ? "var(--sh-emerald)" : rowFitsLimit === false ? "var(--sh-red)" : "var(--sh-fg-muted)" }}>{rowFitsLimit === true ? "Fits limit" : rowFitsLimit === false ? `Over limit · over by ${exactMoney(rowOverageCents)}` : "Limit pending"}</small></span>
              <span className="font-mono tabular-nums">{item.spreadPct == null ? "—" : `${item.spreadPct.toFixed(1)}%`}</span><span className="font-mono tabular-nums">{item.market?.dailyVolume?.toLocaleString() ?? "—"}</span><span className="font-mono tabular-nums">{item.contract.openInterest?.toLocaleString() ?? "—"}</span>
            </button>;
          })}
        </div> : <div className="rounded-md border px-3 py-3 text-xs leading-5" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}><strong style={{ color: "var(--sh-text-primary)" }}>{optionAvailability.title}</strong><br />{optionAvailability.summary}{optionChain.error?.message && <><br /><span className="font-mono">Provider detail: {optionChain.error.message}</span></>}</div>}
