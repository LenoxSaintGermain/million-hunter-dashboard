  ...
  return <Card id="paper-proposal" className="scroll-mt-6 border" style={{ borderColor: "var(--sh-signal)", background: "var(--sh-surface-2)" }}>
    <CardContent className="space-y-4 pt-4">
      ...
      {isOption && <section ref={contractPickerRef} aria-labelledby="contract-picker-heading" className="scroll-mt-5 space-y-3 rounded-lg border p-3" style={{ borderColor: "var(--sh-signal)", background: "var(--sh-surface)" }}>
        ...
        {optionChain.isFetching ? <div className="flex min-h-20 items-center justify-center gap-2 rounded-md border text-xs" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}><Loader2 className="h-4 w-4 animate-spin" />Loading live contracts…</div> : optionChainItems.length ? <div className="overflow-hidden rounded-md border" style={{ borderColor: "var(--sh-border-1)" }}><div className="hidden grid-cols-[1.05fr_1.3fr_.9fr_.6fr_.55fr_.55fr] gap-2 border-b px-3 py-2 text-[9px] font-semibold uppercase tracking-[0.1em] sm:grid" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}><span>Contract</span><span>Bid / ask · mark</span><span>Maximum loss</span><span>Spread</span><span>Volume</span><span>Open int.</span></div>{optionChainItems.map((item) => { const selected = optionContractSymbol === item.contract.symbol; const rowMaxLossCents = item.market?.askPriceCents != null && Number.isInteger(optionQty) && optionQty > 0 && slippageCents != null ? optionQty * 100 * (item.market.askPriceCents + slippageCents) : null; const rowFitsLimit = rowMaxLossCents != null && visibleRiskCeilingCents != null ? rowMaxLossCents <= visibleRiskCeilingCents : null; const rowOverageCents = rowFitsLimit === false && rowMaxLossCents != null && visibleRiskCeilingCents != null ? rowMaxLossCents - visibleRiskCeilingCents : null; return <button key={item.contract.symbol} type="button" disabled={!item.quoteReady} aria-pressed={selected} aria-label={`${candidate.symbol} ${price(item.contract.strikePriceCents)} ${item.contract.type}; ${item.quoteReady ? `bid ${price(item.market?.bidPriceCents)}, ask ${price(item.market?.askPriceCents)}, maximum loss ${money(rowMaxLossCents)}, ${rowFitsLimit === true ? "fits limit" : rowFitsLimit === false ? `over limit by ${exactMoney(rowOverageCents)}` : "risk limit unavailable"}` : "quote unavailable"}`} onClick={() => selectQuotedContract(item)} className="grid min-h-12 w-full gap-1 border-b px-3 py-2 text-left text-xs last:border-b-0 disabled:cursor-not-allowed disabled:opacity-50 sm:grid-cols-[1.05fr_1.3fr_.9fr_.6fr_.55fr_.55fr] sm:items-center" style={{ borderColor: "var(--sh-border-1)", background: selected ? "color-mix(in srgb, var(--sh-signal) 10%, var(--sh-surface))" : "transparent", color: "var(--sh-text-primary)" }}><span className="font-semibold tabular-nums">{price(item.contract.strikePriceCents)} {item.contract.type}</span><span className="font-mono tabular-nums">{item.market ? <>{price(item.market.bidPriceCents)} / {price(item.market.askPriceCents)}<small className="ml-1 opacity-70">· mark {price(item.midpointCents)}</small></> : "Quote unavailable"}</span><span className="font-mono tabular-nums">{exactMoney(rowMaxLossCents)}<small className="ml-1 font-sans font-semibold" style={{ color: rowFitsLimit === true ? "var(--sh-emerald)" : rowFitsLimit === false ? "var(--sh-red)" : "var(--sh-fg-muted)" }}>{rowFitsLimit === true ? "Fits limit" : rowFitsLimit === false ? `Over limit · over by ${exactMoney(rowOverageCents)}` : "Limit pending"}</small></span><span className="font-mono tabular-nums">{item.spreadPct == null ? "—" : `${item.spreadPct.toFixed(1)}%`}</span><span className="font-mono tabular-nums">{item.market?.dailyVolume?.toLocaleString() ?? "—"}</span><span className="font-mono tabular-nums">{item.contract.openInterest?.toLocaleString() ?? "—"}</span></button>; })}</div> : <div className="rounded-md border px-3 py-3 text-xs leading-5" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}><strong style={{ color: "var(--sh-text-primary)" }}>No usable live contracts returned.</strong><br />{optionChain.data?.unavailableReason ?? optionChain.error?.message ?? "Retry the chain or preserve cash; do not guess a contract."}</div>}
