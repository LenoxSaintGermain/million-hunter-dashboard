...
export function ArgumentRail({ state, operatorCapCents, evidenceLabel, gateLabel, onDiagnosticSelect }: { state: WorkflowState; operatorCapCents: number | null; evidenceLabel: string; gateLabel: string; onDiagnosticSelect: (target: "thesis" | "evidence" | "mechanism" | "risk" | "gate") => void }) {
  const nodes: Array<{ key: "thesis" | "evidence" | "mechanism" | "risk" | "gate"; title: string; state: WorkflowState; detail: string; next: string }> = [
    { key: "thesis", title: "Thesis", state: "rule_qualified", detail: "Thesis bound", next: "Review the assigned thesis" },
    { key: "evidence", title: "Evidence", state: evidenceLabel === "—" ? "unknown" : state === "conditional" ? "conditional" : "researchable", detail: evidenceLabel === "—" ? "Built after research starts" : evidenceLabel, next: "Go to the decision outcome" },
    { key: "mechanism", title: "Mechanism", state: "unknown", detail: "Chosen on the Play Slate", next: "Go to the decision outcome" },
    { key: "risk", title: "Risk", state: operatorCapCents == null ? "unknown" : "rule_qualified", detail: operatorCapCents == null ? "Enter planned loss" : "Risk cap declared", next: "Go to Mission Math" },
    { key: "gate", title: "Gate", state, detail: gateLabel, next: "Go to the decision outcome" },
  ];
  return <section aria-label="Decision argument rail" className="overflow-hidden rounded-xl border" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface)" }}><div role="list" className="grid grid-cols-2 gap-px sm:grid-cols-5" style={{ background: "var(--sh-border-1)" }}>{nodes.map((node, index) => <div role="listitem" key={node.title} className={index === nodes.length - 1 ? "col-span-2 sm:col-span-1" : ""}><button type="button" className="min-h-20 w-full p-3 text-left transition-colors hover:bg-black/[0.025] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset" style={{ background: "var(--sh-surface-2)" }} aria-label={`${node.title}: ${node.detail}. What to do next: ${node.next}`} onClick={() => onDiagnosticSelect(node.key)}><p className="text-[0.58rem] font-semibold uppercase tracking-[0.12em]" style={{ color: "var(--sh-fg-muted)" }}>{index + 1}. {node.title}</p><div className="mt-1.5 sm:mt-2"><StateMark state={node.state} compact /></div><p className="mt-1 text-[11px] leading-4" style={{ color: "var(--sh-text-primary)" }}>{node.detail}</p><span className="sr-only">What to do next: {node.next}</span></button></div>)}</div></section>;
}
...
export function RiskBudgetBar({ operatorCapCents, perPlayCeilingCents, accountEquityCents, accountMandatePct, concentrationBlocked }: { operatorCapCents: number | null; perPlayCeilingCents: number | null; accountEquityCents?: number | null; accountMandatePct?: number | null; concentrationBlocked: boolean }) {
  ...
  if (resolved.effectiveLimitCents == null) return <section className="rounded-xl border border-dashed p-4" style={{ borderColor: "var(--sh-border-1)", color: "var(--sh-fg-muted)" }}><div className="flex items-center gap-2"><StateMark state="unknown" compact /><BasisMark basis="unknown" label="Risk limit —" /></div><p className="mt-2 text-xs leading-5">Enter Max planned loss above $0 to calculate the effective ticket limit. This is an unconfigured mission, not a Preserve Cash decision.</p></section>;
