...
export function RiskBudgetBar({ operatorCapCents, perPlayCeilingCents, concentrationBlocked }: { operatorCapCents: number | null; perPlayCeilingCents: number | null; concentrationBlocked: boolean }) {
  ...
  return <section className="rounded-xl border p-4" style={{ borderColor: concentrationBlocked ? "var(--sh-red)" : mandateControls ? "var(--sh-signal)" : "var(--sh-border-1)", background: "var(--sh-surface-2)" }}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.14em]" style={{ color: "var(--sh-fg-muted)" }}>Effective maximum loss</p><p className="mt-1 font-serif text-2xl" style={{ color: "var(--sh-text-primary)" }}>{formatMoney(resolved.effectiveLimitCents)}</p></div><BasisMark basis="calculated" label={mandateControls ? "Account mandate controls" : "Your lower limit controls"} formula="smaller of mission limit and account mandate" /></div><div className="mt-3 grid grid-cols-2 overflow-hidden rounded-lg border text-xs" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface)" }}><div className="border-r p-3" style={{ borderColor: "var(--sh-border-1)" }}><p style={{ color: "var(--sh-fg-muted)" }}>You requested</p><p className="mt-1 font-mono font-semibold tabular-nums" style={{ color: "var(--sh-text-primary)" }}>{formatMoney(operatorCapCents)}</p></div><div className="p-3"><p style={{ color: "var(--sh-fg-muted)" }}>Account mandate</p><p className="mt-1 font-mono font-semibold tabular-nums" style={{ color: "var(--sh-text-primary)" }}>{formatMoney(perPlayCeilingCents)}</p></div></div><p className="mt-2 text-xs leading-5" style={{ color: "var(--sh-fg-muted)" }}>The smaller limit controls every ticket. Increasing mission capital does not raise the account mandate.</p>{concentrationBlocked && <div className="mt-2"><StateMark state="blocked" label="Blocked by concentration" compact /></div>}</section>;
}
