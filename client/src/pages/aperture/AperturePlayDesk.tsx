  ...
  return <DashboardLayout><div className="mx-auto max-w-6xl space-y-5 pb-12">
    ...
    {!isLoading && (inMotionOrders.length > 0 || activePlays.length > 0) && <section className="space-y-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">{visibleOrders.map((order) => {
        const state = orderState(order);
        return <article key={`order-${order.id}`} className="min-w-0 rounded-xl border p-4" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface)" }}>
          <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><Badge variant="outline">{instrumentLabel(order.instrumentType)}</Badge><h3 className="mt-2 break-all font-mono text-xl font-semibold tabular-nums" style={{ color: "var(--sh-text-primary)" }}>{order.symbol}</h3></div><span className="max-w-[10rem] shrink-0 text-right text-xs font-medium leading-4" style={{ color: "var(--sh-emerald)" }}>{state.label}</span></div>
      ...
      })}{visibleActivePlays.map((play) => <article key={`play-${play.id}`} className="min-w-0 rounded-xl border p-4" style={{ borderColor: "var(--sh-border-1)", background: "var(--sh-surface)" }}><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><Badge variant="outline">{instrumentLabel(play.instrumentType)}</Badge><h3 className="mt-2 break-all font-mono text-xl font-semibold tabular-nums">{play.symbol}</h3></div><span className="shrink-0 text-xs font-medium" style={{ color: "var(--sh-signal)" }}>{play.status === "watching" ? "Watching" : "Active"}</span></div><p className="mt-4 text-xs" style={{ color: "var(--sh-fg-muted)" }}>{play.horizon ?? "No review horizon declared"}</p><Button size="sm" variant="outline" className="mt-3" onClick={() => navigate("/aperture/accounts")}>Open portfolio<ArrowRight className="ml-1.5 h-3.5 w-3.5" /></Button></article>)}</div>
