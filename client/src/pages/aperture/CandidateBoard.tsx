...
export default function CandidateBoard() {
  ...
  const reviewEvidence = trpc.aperture.run.evidence.review.useMutation({
    onSuccess: () => { setReviewProgressMessage("Review recorded. The decision path has been updated below; no paper order was created."); toast.success("Human review recorded. This does not create a paper order."); refetch(); },
    onError: (error) => toast.error(error.message),
  });
