...
describe("Decision Runway client revision contract", () => {
  it("appends conditional and cash revisions but opens a new Decision Run for fresh research", () => {
    const source = readFileSync(
      resolve(process.cwd(), "client/src/components/aperture/DecisionRunway.tsx"),
      "utf8",
    );

    expect(source).toContain('&& !(branch === "research" && runway.latest.runId != null)');
