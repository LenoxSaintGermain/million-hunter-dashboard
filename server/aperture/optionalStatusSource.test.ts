...
describe("partial source failure", () => {
  it.each(["Monitoring", "Discovery receipt"])("preserves useful records after %s failure without a quiet empty result", async source => {
    const failed = await readOptionalStatusSource(source, async () => { throw new Error("connection lost"); });
